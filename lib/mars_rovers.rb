...
class MarsRovers
  ...
  def relative_direction_validator(line)
    relative_direction = line_splitter(line)
    position           = [relative_direction[0].to_i, relative_direction[1].to_i]
    direction          = relative_direction[2]
    position.all?(Integer) && CARDINAL_POINTS.include?(direction) ? true : error_panel
  end

  def rel_direction_sizer(line)
    line_splitter(line).length == 3
  end

  def rover_x_coordinate_validator(line)
    rover_x_coordinate = line_splitter(line)[0].to_i
  ...
  def rover_y_coordinate_validator(line)
    rover_y_coordinate = line_splitter(line)[1].to_i
  ...
  def rovers_validator
    ...
    rovers.each_with_index do |line, index|
      ...
      rel_direction_values  << relative_direction_validator(line)
